...
double Function::SourceFunction(const double x, const double y, const double t) const
{
  ...
  - 4*M_PI*(-_ymin + y)*pow(t+1,2)*sin(_ymax - y)*cos(2*M_PI*(-_xmax + x))
  + (-_xmin + x)*(-_ymin + y)*pow(t+1,2)*sin(2*M_PI*(-_xmax + x))*sin(_ymax - y)
  + 2*(-_xmin + x)*pow(t+1,2)*sin(2*M_PI*(-_xmax + x))*cos(_ymax - y))
   + (-_xmin + x)*(-_ymin + y)*(-2*t - 2)*sin(2*M_PI*(-_xmax + x))*sin(_ymax - y);
}
